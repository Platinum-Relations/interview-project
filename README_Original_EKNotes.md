...
| `gross_amount`    | The full amount, **before fees**. Negative for refunds.                                                                                                                                                          | Only use Java `BIGDECIMAL` internally for these, and an equivalent BCD type in SQL DBs in case someone later uses a trigger to push txns to the data lake or an event steam, and things get rounded. |
| `currency`        | `USD`                                                                                                                                                                                                            |                                                                                                                                                                                                      |
| `type`            | `SALE` or `REFUND`                                                                                                                                                                                               | This table must be for *financial transactions* (transactions that make it into the batch) only.                                                                                                     |  
| `captured_at`     | ISO 8601, when we captured the payment                                                                                                                                                                           | Store this as a SQL DATE type or `long` in docDBs that aren't fast at sorting dates or have poor i18n functionality.                                                                                 |

### `processor_settlement.json` - the processor (JSON array)

| Field                                    | Notes                                                                         | EK Notes                                                                                                                                                                                             |
| ---------------------------------------- | ----------------------------------------------------------------------------- |------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
| `settled_amount`                         | The **net** amount actually settled (gross minus fees). Negative for refunds. | Only use Java `BIGDECIMAL` internally for these, and an equivalent BCD type in SQL DBs in case someone later uses a trigger to push txns to the data lake or an event steam, and things get rounded. |
| `interchange_fee`, `processor_fee`       | The fees the processor deducted                                               | Only use Java `BIGDECIMAL` internally for these, and an equivalent BCD type in SQL DBs in case someone later uses a trigger to push txns to the data lake or an event steam, and things get rounded. |
| `currency`                               |                                                                               | ISO Currency Trigraph                                                                                                                                                                                |
| `settlement_date`                        | The date it settled - **typically 1–3 days after** `captured_at`              | Store this as a SQL DATE type or long in docDBs that aren't fast at sorting dates or have poor i18n functionality.                                                                                                                                                                                                     |
...
Round HALF_UP is typical for USA. Was a Globalization Engineer and had to push code through Eurozone BASDA audits in the past.
...
### EK Notes
What you call `ORPHAN REFUND` was classically called a `RETURN` or today more commonly `BLIND REFUND`.
Whatever you want to call it, it's a *different TRAN TYPE* (in the merchant and your front-end API context) and is or 
isn't allowed according to certain criteria (not getting into all that), but usually the `MERCHANTBOARDING RECORD` will
have a toggle/boolean for `ALLOW_BLIND_REFUNDS` or similar so that the merchant themselves can control this. 

For T&E and Restaurant merchants, or anywhere else (Car Rental, Cruise, Retail, etc.) that you are managing Merchant 
Boarding Accounts with `CORPORATE` vs. `FRANCHISE` merchants of the same name/flag (e.g. Hilton, Westin, McDonalds, etc.), 
there might be stipulated `default values` for all merchants within those classifications...

I'll do this, but the usual way is: match up your transaction reports to your merchant transactions/batches, then
...
### EK Notes
Got it, typical "Count and Amount" type stuff without the multi-currency or multi-tender-type (`CREDIT` vs `DEBIT`) and 
Instant (Single Message) vs. Eventual (Dual Message) breakdowns.</span>
...
### EK Notes
I don't know how to understand this. I would need to ask follow ups.<br>
<br>
NOTE: I am reading the 
...
matches it.<br>
1. Is this in reference to the breakdowns present in the TPP Settlement Reports?
   2. As in the `CAPTURE` of the `AUTH` being priced in a separate section from the `AUTH` itself, and how to knit fees together for these 2 things?
3. Is this in reference to literal dupe rows of the `CAPTURE` where `Interchange % Fee` (Ad Valorem or however else you call it) is on one row, and other fees (like `Interchange Amount`) are on separate, dupe-looking lines?
<br><br>
...
- **Tests** - the core matching and fee logic is covered.
  - <span style="color: red;">Hope you don't care if I use Mockito, JUnit, TestNG, etc.?
    - I like TestNG soft asserts and Mockito's ability to let me unit-test private methods
...
- Handling of the open questions above (tolerance, date window, split settlements).
  - In the "Good Old Days", PAN, Refno (Merchant ref or STAN, etc.), and amount were enough for disambiguation
  - AMEX and others allowed identical transactions for high dollar amounts exceeding their amount maximum (over $99,999 for instance)
    - You used to divide amount x 2, create 2 identical transactions, and set the FORCE FLAG when doing AMEX DIRECT AUTH or most TPPs
  - I'll try and stick with this, because it's easy to reason through.
...
### EK Notes
- Graceful degradation I am big on, but there are some functions where FAIL_STOP behavior is best.
  - Producing an exception report while still moving along or processing records is one way to do this
- I get trying to prevent batches stuck in suspense at the downstream HOST is a real thing, but there are "Worse Things (TM)"
    - Stops the need for manual intervention by skilled people, which eventually results in those records removed from batch *anyway*
- "Worse Things" might be the need for front-end reconciliation against your own host, providing adequate UI functions and a somewhat understandable UX for dealing with records that are plainly
  - Done during the shift/batch
  - Part of the day's/shift's/cashier's/whatever batch
  - Not funded
  - At minimum you need UI and back-end support (in case the merchant calls for it) for
    - Re-submission of the flawed transaction(s)
    - Putting Humpty Dumpty (the merchant batch in this case) back together again by either
      - Re-adding the transaction the same merchant batch (with special statuses showing it was a resubmission)
      - Creating a special "new" /"parallel" batch just for resubmissions, linking it to the merchant batch, *plus* linking it to the new batch at the TPP/HOST where it actually *did* get settled
    - Now repeat the above for your *funding* (not just settlement/clearing) logic
      - Add in what the policy is for fees where Next Day/Same Day settlement didn't work
      - Add in how your backend maintains merchant_batch to TPP_batch/TPP_sub_batch to PayFac_funding linkages
    - And then whatever special sauce you need for internal support of *chargebacks* on those transactions, because any changes made to amount or other significant data might enable the cardholder to initiate on       
    - This all needs **wargaming out** for a new gateway/payfac build
    - Not just devs, but product owner and pricing specialist need to be involved
